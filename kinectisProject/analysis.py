...
def findPrevailingMechanism(size,inWt,fiWt,time):
    #######Taking Inputs
	initialWeight = float(inWt)
	size = 0
	finalWeights = []
	timePeriods = []
	alpha = []
	...
	finalList = [line.split(',') for line in fiWt.readlines()]
 
	for i in range(0,size):
		ele = finalList[0][i]
		finalWeights.append(float(ele))

	timeList = [line.split(',') for line in time.readlines()]
	...
	calculationOfModifiedGAlpha(gAlpha,modifiedGAlpha,timePeriods,regressionEq,size)
 
	# 6. Function to find modified Alpha
	modifiedAlpha = [[0 for i in range(size)] for j in range(17)]
	calculationOfModifiedAlpha(modifiedGAlpha,modifiedAlpha,size)


	#7. Function to find T Distribution value 
	calculationOfT_test(modifiedAlpha,size,alpha,tVal)
	
	
	#8. Function to find F Distribution value 
	calculationOfF_test(modifiedAlpha,size,alpha,fVal)
 
	maxSum = 100000
	number = -1
	for i in range(0,17):
		if ((tVal[i]+fVal[i])<maxSum):
			maxSum = tVal[i]+fVal[i]
			number = i+1
...
def dataAnalysis(request):
    try:
        if request.method == "POST":
            ...
            reactionData = collection.find({'reaction':reaction,'temperature':temperature})
            size = reactionData["size"]
            initialWeight = reactionData["initialWeight"]
            finalWeights = reactionData["finalWeights"]
            timePeriods = reactionData["timePeriods"]
            Mechanism = findPrevailingMechanism(size,initialWeight,finalWeights,timePeriods)
            messages.success(request,"Mechanism number {Mechanism} is prevailing")
            
    except:
        messages.error(request,"Can not find prevailing Mechanism for this Reaction")
